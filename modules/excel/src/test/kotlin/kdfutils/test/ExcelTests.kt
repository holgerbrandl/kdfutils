package kdfutils.test

import com.github.holgerbrandl.kdfutils.CompactColumnSpec
import com.github.holgerbrandl.kdfutils.NamedColumnSpec
import com.github.holgerbrandl.kdfutils.readExcel
...
class ExcelTests {
    ...
    private val testReadingPath = File("$testResourcePath/ExcelReadExample.xlsx")
    private val testWritingPath = File("$testResourcePath/ExcelWriteResult.xlsx")
    ...
    fun `readExcel -  should read excel file`() {
        ...
        df.columnNames() shouldBe listOf("Name", "Email", "BirthDate", "Country", "Activities", "Registered")
...
//    }
    ...
    fun `readExcel - shrt colTypes should work`() {
        val df = DataFrame.readExcel(
            File("$testResourcePath/colspec.xlsx"),
             colTypes = CompactColumnSpec("dss")
        )

        (df["type"].isType<Double>()) shouldBe true // Tests when ColType is given
        (df["smthg"].isType<String>()) shouldBe true // Tests when ColType is given
